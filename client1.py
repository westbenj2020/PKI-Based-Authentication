...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as applicationServer:
    ...
    #  Constructed message contents:
    #  [req||Timestamp7]
    des_cipher7 = DES.new(extractedSessionKey6.encode("utf-8"), DES.MODE_OFB)
    timestamp7 = time.time().__trunc__()
    message7 = req + timestamp7.__str__()
    encodedMessage7 = message7.encode("utf-8")
    encodedMessage7Encrypted = des_cipher7.encrypt(encodedMessage7)
    ...
    print('\nReceived ciphertext from application server: "{}"'.format(receivedData8))
    des_cipher8 = DES.new(extractedSessionKey6.encode("utf-8"), DES.MODE_OFB)
    receivedData8Decrypted = des_cipher8.decrypt(receivedData8)
    receivedData8DecryptedDecoded = receivedData8Decrypted.decode("utf-8")
